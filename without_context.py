...
def train_model(algo, algo_name, log_dir, contexts=contexts, hide_context=True, seed=None):
...
def test_model(log_dir, algo, test_lengths):
    ...
    test_env = CARLPendulum(contexts=test_context, hide_context=True, dict_observation_space=False)
...
for i, seed in enumerate(seeds, start=0):
    log_dir = f"{sac_log_dir}/sac_without_context_seed_{i}"
    train_model(SAC, "sac_without_context", log_dir, contexts, seed=seed)
